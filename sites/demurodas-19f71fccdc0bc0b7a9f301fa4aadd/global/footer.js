...
if (pagePath.includes('seating')) {
  category = "Seating";
  subcategory = "Chairs";
} else if (pagePath.includes('all')) {
  category = "All";
} else if (pagePath.includes('new')) {
  category = "New";
}
  ...
  $.get("/dev/all-wip-collection", function(data) {
    ...
    Promise.all(loadPromises).then(results => {
      ...
      dataArray = results.filter(res => {
        if (res && category == 'All') {
          return res
        } else if (res && category == 'New') {
          return res.new;
        } else if (res && (res.category === category && res.subcategory == subcategory)) {
          return res
        }
      });      
